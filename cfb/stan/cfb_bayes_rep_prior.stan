  ...
  vector[nPlayers] bpmMean        ; //prior mean is equal to bpm
  vector[nPlayers] bpmVar        ; // prior rpm variance based off bpm
  ...
  b ~ student_t(8, pmReplace, sqrt(s2player)*positionVarCoef) ;

// soft constraint to make positions weights sum to approximately 1
  sumWeightedPhi ~ normal(1, sdSumWeights*nPosition);

  s2player ~ gamma(0.5 , 45); // in a later version I may be able to change 0.03 and 10 to their own parameters
